...
- **`wire send` routes over Nostr when no HTTP slot is reachable** (#227 D3.4, RFC-007): the send-path half of daemon routing. `send::attempt_deliver` (the single chokepoint behind `wire send`, group send, MCP `tool_send`, and probes) now falls back to the peer's recorded `nostr_transport` when no HTTP endpoint reaches them — it encodes the *same* signed wire event as a NIP-01 event (`nostr_event::wire_to_nostr_addressed`, full inner Ed25519 sig intact, `p`-tagged to the peer's npub, schnorr-signed by our `wire enroll nostr` key) and publishes it to the paired relay. New `SyncDelivery::DeliveredNostr` verdict (`status: "delivered_nostr"`, carries `transport: "nostr"` + the peer's npub; counts as relay-reached so the peer can `wire nostr fetch` it). **Strictly a fallback** — when the peer has no `nostr_transport` (or this session isn't enrolled) the HTTP-slot path is byte-identical to before. The pull-loop side (daemon auto-pulls Nostr-delivered events into the inbox) is the next slice.
- **Nostr peer reachability recorded on accept** (#227 D3.4, RFC-007): `wire nostr accept` now records the peer's Nostr transport coords — their npub + the `wss://` relay — under `peers[<handle>].nostr_transport` in relay-state (additive `endpoints::pin_peer_nostr_transport` / `peer_nostr_transport`; preserved across HTTP-endpoint re-pins per the #162 monotonic-state rule). Reachability only — trust is the separate VERIFIED pin. This is the data foundation for routing a `wire send` over Nostr; the send-path + pull-loop routing (and recording the initiator's side from the pair-ack) are the next slices. No behavior change to the HTTP-slot path.
- **`wire nostr accept` — complete a bilateral pair over Nostr** (#227 D3.4, RFC-007): the consent gate for Nostr pairing. `wire nostr accept <npub> --relay wss://…` pulls the peer's NIP-W1 pair-request, **verifies the card** (signature) **and its nostr binding resolves to the exact npub that sent it** (so a relayed copy of someone else's card can't be accepted), pins the peer **VERIFIED** (the explicit accept is the consent — same gate semantics as `wire accept`, #245 collision guard applies), and sends a pair-ack back. This closes the bilateral trust handshake over a public relay (pair → accept → ack); routing regular wire messages over Nostr (vs the trust pin) remains the deferred daemon-routing step.
- **`wire nostr` CLI — pair + fetch over a Nostr relay** (#227 D3.4, RFC-007): the runnable capstone of the Nostr stack. `wire nostr pair <npub> --relay wss://…` sends an encrypted NIP-W1 pair-request (your signed card) to a peer's npub; `wire nostr fetch --relay wss://…` pulls + decrypts the events addressed to your npub (pair-requests, pair-acks, messages) and lists who they're from. Uses this session's secp transport key (`wire enroll nostr`); errors clearly if not enrolled. Ties together binding + codec + relay protocol + websocket + NIP-44 + NIP-W1 into something an operator can actually run against a live relay (the manual D3.4 dogfood path). Deliberately additive — does NOT touch the daemon send/pull hot path; auto-routing `transport: nostr` peers from the daemon remains the deferred integration.
- **NIP-W1 wire pairing over Nostr — no-SPAKE2 codec** (#227 D3.4, RFC-007): pairing over Nostr without reintroducing the SPAKE2/SAS flow wire removed post-RFC-005 (operator decision). `nip_w1::build_pair_event` seals an agent's signed card (+ D3.1 binding) as a **NIP-44-encrypted** Nostr event (`kind 21050` request / `21051` ack), `p`-tagged to the peer's npub and schnorr-signed by the sender's transport key; `open_pair_event` verifies the transport (id + schnorr), NIP-44-decrypts (only the addressee's key works — symmetric ECDH), and returns the sender's card for the existing **bilateral `wire accept`** consent gate to act on. No PAKE/SAS: identity = the signed cards + cross-signed bindings, trust = the explicit accept. Adds `nostr_event::verify_transport` (content-agnostic id+sig check). Offline + 5 unit tests (A→B + ack roundtrip, third-party-can't-decrypt, tampered-content fails transport, non-pairing-kind rejected). The receiver consent-gate wiring + daemon routing integration remain the deferred slices.
- **NIP-44 interop proven against the official vectors** (#227 D3.3 follow-up, RFC-007): closes the gap D3.3 flagged. The canonical NIP-44 v2 conformance vectors (the `valid` subset of the reference `nip44.vectors.json`, public domain) are vendored at `src/testdata/nip44_official_vectors.json` and asserted **byte-exact** — all 35 conversation-key cases, 10 encrypt/decrypt cases (**including exact ciphertext payloads under fixed nonces**), and 24 padded-length cases pass. wire's NIP-44 implementation now demonstrably interoperates with every other NIP-44 implementation; the D3.3 implementation was correct as written.

- **NIP-44 v2 encrypted DMs — the Nostr confidentiality layer** (#227 D3.3, RFC-007): consumes RFC-006's reserved `enc` slot with a *vetted* spec (the `reuse > build` principle) instead of bespoke crypto. `nip44::encrypt`/`decrypt` between two secp256k1 keys (the D3.1 Nostr transport keys) make a wire DM sent over Nostr confidential to the relay. Faithful to NIP-44 v2: conversation key = `HKDF-Extract(salt="nip44-v2", IKM=ecdh_x)` (symmetric across both parties), per-message keys via `HKDF-Expand(ck, info=nonce)`, ChaCha20 over a length-prefixed power-of-two-padded plaintext (size leaks only a coarse bucket), `HMAC-SHA256(nonce‖ciphertext)` verified in **constant time** before decryption, payload = `base64(0x02‖nonce‖ct‖mac)`. 9 unit tests cover ECDH conversation-key symmetry, cross-party decrypt (the real DM path), tamper→MAC-fail, wrong-key→MAC-fail, the padding table, and version/length rejection. **Honest gap (flagged in-module):** cross-implementation interop is not yet proven against the official `nip44.vectors.json` — that vector test is a required follow-up before claiming wire DMs interop with other NIP-44 implementations. Remaining D3: NIP-W1 pairing + live third-party-relay e2e (D3.4), and wiring the transport into the daemon's send/pull routing.

- **Nostr WebSocket transport — `NostrWs` publishes + pulls over `wss://`** (#227 D3.2b-ii, RFC-007): the network half of the Nostr binding, carrying the NIP-01 messages (D3.2b-i) over a real WebSocket. `NostrWs::connect` opens `ws://`/`wss://`; `publish` sends an `EVENT` and awaits the relay's `OK`; `pull` opens a `REQ` (e.g. a `#p` filter for events addressed to our npub), collects `EVENT`s until `EOSE`, then `CLOSE`s. Each pulled event is **transport-verified** (`nostr_event::verify_and_decode` — recompute NIP-01 id + check the schnorr sig) before it's returned, so a hostile relay can't inject a forged event; the caller still verifies the inner Ed25519 wire signature + the D3.1 binding. Every relay read is timeout-bounded so a silent relay can't wedge the caller. Two `#[tokio::test]` integration tests run against an in-process NIP-01 relay mock (no subprocess): a publish→pull roundtrip, and a hostile-relay test proving tampered events are dropped. New deps: `tokio-tungstenite` 0.29 (rustls/webpki-roots, matching the existing TLS stack). The `Transport` trait unifying this with the synchronous HTTP-slot relay is deferred (RFC-007 §1) until a second async transport justifies the abstraction. Remaining D3: NIP-44 v2 DM (D3.3) + NIP-W1 pairing & live third-party-relay e2e (D3.4).
...
- **MCP `wire_accept` now pins a routable peer + accept flags an unreachable reply path** (#277): two gaps. (1) The MCP accept path still wrote the peer pin in the *flat* `peers[h]={relay_url,slot_id,slot_token}` shape that RFC-006 Part B no longer reads — so a peer accepted over MCP got an empty `endpoints[]` and `wire send` couldn't route to them. It now pins via `endpoints[]` like the CLI path. (2) Both accept paths (CLI `wire accept` + MCP) returned `bilateral_accepted` even when the peer advertised only loopback/same-host endpoints — silently reporting success on a reply path that can't reach a remote peer. They now surface `reply_path_reachable: false` (+ a remediation warning on the CLI) via a new `endpoints::endpoints_are_local_only`, which infers reachability from the URL so a loopback address mislabeled `federation` (the exact #277 case) is still caught.
- **MCP `wire_dial` no longer wipes a peer's reply token on re-dial** (#277, RFC-006 Part B): the MCP dial path read the peer's carry-forward `slot_token` from the flat `peers[h].slot_token` field that Part B (#268) stopped writing, so it always read empty — every MCP re-dial of an already-acked peer reset the reply token to `""`, breaking the reply path until a fresh ack. Both dial paths (CLI + MCP) now read the token through one canonical `endpoints::peer_federation_token` reader so they can't drift apart again; the two other stale peer-flat reads #268 missed (a dead `relay.rs` re-resolve fallback and the `trust.rs` PENDING_ACK check) are removed.
